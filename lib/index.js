...
var DEFAULT_MAX_XML_ENTITY_EXPANSIONS = 2e4;
var Config = import_koishi.Schema.intersect([
  import_koishi.Schema.object({
    ...
    maxXmlEntityExpansions: import_koishi.Schema.natural().min(100).default(DEFAULT_MAX_XML_ENTITY_EXPANSIONS).description("用于兼容内容较复杂的订阅源，默认值为 20000；提高该值会增加解析异常内容时的资源消耗。")
  }).description("请求设置"),
  import_koishi.Schema.object({
  ...
  import_koishi.Schema.object({
    pushInitialItems: import_koishi.Schema.boolean().default(false).description("是否在首次订阅后立即推送最新的一篇文章。"),
    pushAllInitialItems: import_koishi.Schema.boolean().default(false).description("是否在首次订阅时推送订阅源返回的所有历史文章。"),
...
function decodeEntity(entity) {
  ...
  if (code !== void 0) return Number.isNaN(code) || code > 1114111 ? entity : String.fromCodePoint(code);
  return {
    amp: "&",
    apos: "'",
    gt: ">",
...
function parseFeed(xml, feedUrl, maxSummaryLength = 500, maxXmlEntityExpansions = DEFAULT_MAX_XML_ENTITY_EXPANSIONS) {
...
function getLatestItem(items) {
  ...
  for (const item of items) {
    if (!latest || item.publishedAt !== void 0 && (latest.publishedAt === void 0 || item.publishedAt > latest.publishedAt)) {
      latest = item;
    }
  }
  return latest;
}
__name(getLatestItem, "getLatestItem");
function formatUpdate(feed, items, includeSummary, heading = `有 ${items.length} 条新文章`) {
...
function apply(ctx, config) {
  ...
  async function createSubscription(source, sendInitial = sendToTarget) {
    ...
    if (initialItems.length) {
      const heading = config.pushAllInitialItems ? `共有 ${initialItems.length} 篇历史文章` : "最新文章";
